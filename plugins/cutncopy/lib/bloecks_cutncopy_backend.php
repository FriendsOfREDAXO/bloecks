...
class bloecks_cutncopy_backend extends bloecks_backend
{
    ...
    public static function init(rex_extension_point $ep)
    {
        if(rex::getUser())
        {
            ...
            rex_extension::register('STRUCTURE_CONTENT_SLICE_MENU', array('bloecks_cutncopy_backend', 'addButtons'));

            // process any cut or copy call
            rex_extension::register('STRUCTURE_CONTENT_BEFORE_SLICES', array('bloecks_cutncopy_backend', 'process'));

            // insert the "INSERT BLOCK" link into module dropdown
            rex_extension::register('STRUCTURE_CONTENT_MODULE_SELECT', array('bloecks_cutncopy_backend', 'addBlockToDropdown'));

            // post process the clipboard after a slice is inserted into another article
            rex_extension::register('SLICE_ADDED', array('bloecks_cutncopy_backend', 'postProcessClipboard'));
        ...
        if($function === static::plugin()->getName())
        {
            ...
            if(method_exists('bloecks_cutncopy_backend', $action))
            {
                ...
                if($slice)
                {
                    ...
                    $subject.= bloecks_cutncopy_backend::$action($slice);
        ...
        if(static::$clipboard_slice instanceof rex_article_slice)
        {
            if((int) static::$clipboard_slice->getId() === static::getCookie('slice_id', 'int', null))
            {
                ...
                if($action === rex_post('bloecks_cutncopy_action', 'string', null))
                {
                    ...
                    if($action === 'cut' && static::hasModulePerm(rex::getUser(), static::$clipboard_slice->getModuleId()))
                    {
                        // remove slice!
