<?php
/**
 * bloecks_cutncopy class - Cut/copy/paste functionality for slices.
 */
class bloecks_cutncopy extends bloecks_backend
{
    /**
    ...
    /**
     * Initialize the cutncopy functionality in the backend.
     */
    public static function init(rex_extension_point $ep)
    {
        if (rex::getUser()) {
            ...
            rex_extension::register('STRUCTURE_CONTENT_SLICE_MENU', ['bloecks_cutncopy', 'addButtons']);

            // process any cut or copy call
            rex_extension::register('STRUCTURE_CONTENT_BEFORE_SLICES', ['bloecks_cutncopy', 'process']);

            // insert the "INSERT BLOCK" link into module dropdown
            rex_extension::register('STRUCTURE_CONTENT_MODULE_SELECT', ['bloecks_cutncopy', 'addBlockToDropdown']);

            // post process the clipboard after a slice is inserted into another article
            rex_extension::register('SLICE_ADDED', ['bloecks_cutncopy', 'postProcessClipboard']);
        } else {
    ...
    /**
     * Returns the name of the cookie for cutncopy functionality.
     *
    ...
    {
        if (rex::getUser()->hasPerm('bloecks[]') && true === $ep->getParam('perm')) {
    ...
    public static function process(rex_extension_point $ep)
    {
        ...
        if ($function === 'cutncopy') {
            if (!empty($action)) {
                $action .= 'Slice';
            }

            if (method_exists('bloecks_cutncopy', $action)) {
    ...
    {
        if (rex::getUser()->hasPerm('bloecks[]') && true === rex::getUser()->getComplexPerm('modules')->hasPerm($slice->getModuleId())) {
    ...
    /**
     * Prepares the $_REQUEST and the $_POST Arrays and fills it with the data
