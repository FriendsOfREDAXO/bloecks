...
class Backend
{
    ...
    /**
     * Mark backend alert HTML as BLOECKS-owned so that the JS toast handler
     * only converts BLOECKS messages and never touches alerts from other AddOns.
     */
    private static function markBackendMessage(string $message_html): string
    {
        $pattern = '/class="([^"]*\balert\b[^"]*)"/i';
